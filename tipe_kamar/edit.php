...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $kapasitas = (int) $_POST['kapasitas']; // Field Baru
    $jumlah_kamar = (int) $_POST['jumlah_kamar']; // Field Baru
    
    // Validasi sederhana
    if (empty($nama) || $harga <= 0 || $kapasitas <= 0) {
        $error = "Nama Tipe, Harga, dan Kapasitas wajib diisi dengan benar.";
    } else {
        // 2. Update data tipe kamar (Query diperbarui)
        // Tipe data: ssiiii (string, string, int, int, int, int)
        $u = $conn->prepare("UPDATE tipe_kamar SET nama_tipe=?, fasilitas=?, harga=?, kapasitas=?, jumlah_kamar=? WHERE id=?");
        $u->bind_param('ssiiii', $nama, $fasilitas, $harga, $kapasitas, $jumlah_kamar, $id);
        ...
        <div class="card shadow-lg border-0 rounded-4" style="max-width: 600px; margin: 0 auto;">
            ...
            <div class="card-header bg-primary text-white rounded-top-4 p-4">
                <h3 class="mb-0 fw-bold">
                    <i class="bi bi-pencil-square me-2"></i> Edit Tipe Kamar
                </h3>
            </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="kapasitas" id="kapasitas" class="form-control rounded-3" 
                                   value="<?php echo htmlspecialchars($r['kapasitas']); ?>" min="1" required>
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" name="jumlah_kamar" id="jumlah_kamar" class="form-control rounded-3" 
                                   value="<?php echo htmlspecialchars($r['jumlah_kamar']); ?>" min="0" required>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-group input-group-lg">
                            ...
                            <input name="harga" id="harga" type="number" class="form-control rounded-end-3" 
                                   value="<?php echo htmlspecialchars($r['harga']); ?>" required min="1000">
                        </div>
                    </div>
                    
                    <div class="mb-4">
                        <label for="fasilitas" class="form-label fw-semibold">Fasilitas</label>
                        <textarea name="fasilitas" id="fasilitas" class="form-control rounded-3" rows="4"><?php echo htmlspecialchars($r['fasilitas']); ?></textarea>
                    </div>
